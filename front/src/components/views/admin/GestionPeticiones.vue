<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Gestió de Peticions</h1>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              <th
                class="px-4 sm:px-6 py-4 text-left text-xs sm:text-sm font-semibold text-gray-700"
              >
                Centre
              </th>
              <th
              ...
              <th
                class="hidden md:table-cell px-6 py-4 text-left text-sm font-semibold text-gray-700"
              >
                Alumnes
              </th>
              <th
                class="hidden lg:table-cell px-6 py-4 text-left text-sm font-semibold text-gray-700"
              >
                Data
              </th>
              <th
              ...
              <th
                class="px-4 sm:px-6 py-4 text-right text-xs sm:text-sm font-semibold text-gray-700"
              >
                Accions
              </th>
            </tr>
...
const autoAssign = async () => {
  if (
    await alertStore.confirm(
      "Estàs segur que vols realitzar l'assignació automàtica? Això acceptarà totes les peticions pendents.",
    )
  ) {
    try {
      ...
      alertStore.addAlert(
        'success',
        `Assignació automàtica completada. ${result.updatedRequests} peticions assignades.`,
      )
    } catch (error) {
