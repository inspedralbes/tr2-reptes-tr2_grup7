...
export const createApplication = async (req, res) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ error: "No se han seleccionado talleres." });
    }
    ...
    console.error(error);
    if (error.code === '23505') { 
        if (error.constraint === 'school_applications_id_center_year_period_key') {
             return res.status(400).json({ error: "Ya existe una solicitud para este año escolar." });
        }
    }
