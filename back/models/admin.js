...
export const updateRequestStatus = async (id, status) => {
  await db.query(
    "UPDATE center_requests SET status = $1 WHERE id_request = $2",
    [status, id],
  );
};
...
export const setStudentAcceptedStatus = async (
  requestId,
  studentId,
  isAccepted,
  workshopId,
  ...
  if (isAccepted) {
    ...
    await db.query(
      `
            INSERT INTO workshop_enrollments (id_workshop, id_student) 
            VALUES ($1, $2) 
    ...
    await db.query(
      `
            ...
            WHERE id_workshop = $1 AND id_student = $2`,
      [workshopId, studentId],
    );
  }
  // Update interest status
  await db.query(
    "UPDATE student_interest SET status = $1 WHERE id_request = $2 AND id_student = $3",
    [isAccepted ? "CONFIRMED" : "WAITING", requestId, studentId],
  );
};
...
export const getAvailableTeachers = async () => {
  const query = `
      SELECT t.id_user, t.first_name, t.last_name, u.email, u.is_active, c.center_name
      FROM teachers t
      JOIN users u ON t.id_user = u.id
      LEFT JOIN centers c ON t.id_center_assigned = c.id_user
      -- Removed WHERE u.is_active = true to allow seeing inactive teachers in admin panel
    `;
  const result = await db.query(query);
  return result.rows;
};
